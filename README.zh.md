...
本地私有案例库共有 165 个已审计案例。完整语料前六批完成后，已有 124
个案例通过完整案例契约：包括原有 15 个 MVP 案例和新增恢复的 109 个案例。
...
审计记录为 124 个完成、11 个有证据支持的阻塞、30 个待处理；阻塞细分为
6 个 `blocked_source_missing`、3 个 `blocked_ambiguous_mapping` 和 2 个
...
- [ ] 将剩余 30 个私有案例逐个处理为 completed 或有充分证据的 blocked。
...
165 个私有案例中有 124 个满足完整契约，30 个仍为待处理，11 个具有通过
