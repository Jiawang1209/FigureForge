...
本地私有案例库共有 165 个已审计案例。完整语料前三批完成后，已有 75
个案例通过完整案例契约：包括原有 15 个 MVP 案例和新增恢复的 60 个案例。
...
带组成扇形的世界采样地图。带渲染审计记录为 75 个完成、0 个阻塞、90 个
待处理。由于尚未获得再分发许可，165 个案例均为 `private_only`。
...
- [ ] 将剩余 90 个私有案例逐个处理为 completed 或有充分证据的 blocked。
...
165 个私有案例中有 75 个满足完整契约，90 个仍为待处理，当前没有案例
被归为阻塞。公开精选案例集仍在整理；MCP Server 仍是规划项，在 Skill
与案例库最终验收前保持暂停且尚未实现。
