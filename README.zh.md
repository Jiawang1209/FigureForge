...
本地私有案例库共有 165 个已审计案例。完整语料前五批完成后，已有 110
个案例通过完整案例契约：包括原有 15 个 MVP 案例和新增恢复的 95 个案例。
...
模块树、代谢物 IQR 点区间图和 GO 重排彩色标签。带渲染审计记录为 110 个
完成、5 个有证据支持的阻塞、50 个待处理；阻塞细分为 3 个
`blocked_source_missing`、1 个 `blocked_visual_reference` 和 1 个
`blocked_ambiguous_mapping`。由于尚未获得再分发许可，165 个案例均为
`private_only`。
...
- [ ] 将剩余 50 个私有案例逐个处理为 completed 或有充分证据的 blocked。
...
165 个私有案例中有 110 个满足完整契约，50 个仍为待处理，5 个具有通过
校验的案例级阻塞记录。公开精选案例集仍在整理；MCP Server 仍是规划项，
在 Skill 与案例库最终验收前保持暂停且尚未实现。
