...
if [ -n "${FIGUREFORGE_V101_OUTPUT_DIR:-}" ]; then
  VERIFY_ROOT=$FIGUREFORGE_V101_OUTPUT_DIR
  if [ -e "$VERIFY_ROOT" ] &&
  ...
  VERIFY_ROOT=$(mktemp -d /tmp/figureforge-v101.XXXXXX)
fi
...
ARCHIVE="$VERIFY_ROOT/figureforge-skill-1.0.1.tar.gz"
MANIFEST="$VERIFY_ROOT/figureforge-skill-1.0.1-manifest.csv"
CHECKSUM="$ARCHIVE.sha256"
INSTALL_ROOT="$VERIFY_ROOT/installed-project/.agents/skills"
INSTALLED="$INSTALL_ROOT/figureforge"

stage "FigureForge R tests (excluding recursive v1 acceptance wrapper)"
...
[ "$(sed -n '1p' "$INSTALLED/VERSION")" = "1.0.1" ]
...
stage "v1.0.0 to v1.0.1 upgrade compatibility"
...
if [ "$RUN_LIVE" = "1" ]; then
  ...
  bash "$REPO_ROOT/scripts/run_figureforge_live_evals.sh" \
    --output-dir "$VERIFY_ROOT/live-evals"
else
  stage "bounded live trigger probes skipped"
  echo "Set FIGUREFORGE_RUN_LIVE_EVALS=1 to enable the explicit 1/1 and implicit 9/10 release gate."
fi
...
echo "FigureForge Skill v1.0.1 acceptance: PASS"
